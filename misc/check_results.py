...
if len(sys.argv) != 4:
	print(f"ERROR: Not provided 3 required arguments - {str(sys.argv)}")
	exit(1)
...
if str(file_count) != files_required:
	print(
		f"Current files: {str(file_count)}, but required is: {str(files_required)}"
	)
...
if str(directory_count) != directories_required:
	print(
		f"Current directories: {str(directory_count)}, but required is: {str(directories_required)}"
	)
	print("This commit probably introduced regression, please recheck it.")
	exit(1)
